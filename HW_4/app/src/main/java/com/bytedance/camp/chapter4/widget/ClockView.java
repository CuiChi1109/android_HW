...
public class ClockView extends View {
    ...
    private static final float NUMBER_LENGTH_RATIO = 0.8f;
    ...
    private void init() {
        ...
        numberPaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        //秒针
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        double second_radians = Math.toRadians(-(450-second*6)%360);
        float second_needle_X = (float)(centerX + SECOND_NEEDLE_LENGTH_RATIO * radius * Math.cos(second_radians));
        float second_needle_Y = (float)(centerY + SECOND_NEEDLE_LENGTH_RATIO * radius * Math.sin(second_radians));
        canvas.drawLine(centerX, centerY, second_needle_X, second_needle_Y, needlePaint);

        //分针
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        double minute_radians = Math.toRadians(-(450-minute*6)%360);
        float minute_needle_X = (float)(centerX + MINUTE_NEEDLE_LENGTH_RATIO * radius * Math.cos(minute_radians));
        float minute_needle_Y = (float)(centerY + MINUTE_NEEDLE_LENGTH_RATIO * radius * Math.sin(minute_radians));
        canvas.drawLine(centerX, centerY, minute_needle_X, minute_needle_Y, needlePaint);

        //时针
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        double hour_radians = Math.toRadians(-(450-hour*30-minute/2)%360);
        float hour_needle_X = (float)(centerX + HOUR_NEEDLE_LENGTH_RATIO * radius * Math.cos(hour_radians));
        float hour_needle_Y = (float)(centerY + HOUR_NEEDLE_LENGTH_RATIO * radius * Math.sin(hour_radians));
        canvas.drawLine(centerX, centerY, hour_needle_X, hour_needle_Y, needlePaint);

        Log.i("run_1", "run");
        Log.i("position_1", String.valueOf(centerX)+","+String.valueOf(centerY)+","+String.valueOf(second_needle_X)+","+String.valueOf(second_needle_Y));
        Log.i("time_1", "second:"+String.valueOf(second)+", min:"+String.valueOf(minute)+", hour:"+String.valueOf(hour));
        /**
         * 思路：
    ...
    private void drawTimeNumbers(Canvas canvas) {
        // TODO 绘制表盘时间数字（可选）
        for(int i = 1; i <= 12; i++){
            double radians = Math.toRadians(-(450-i*30)%360);
            float number_X = (float)(centerX + NUMBER_LENGTH_RATIO * radius * Math.cos(radians));
            float number_Y = (float)(centerY + NUMBER_LENGTH_RATIO * radius * Math.sin(radians));
            String number;
            if(i<10){
            ...
            canvas.drawText(number, number_X, (int)(number_Y+rect.height()/2), numberPaint);
        }
    }
